function omegas2 = PDController2(QuadCopter, Target)
    ...
    if(numel(Target) == 3)
        nFB     = (m*g + P(3)*(Target(3) - s(3)) + (kd(3,3) - D(3))*s(9))/(cos(s(4))*cos(s(5))); %thrust
        theta = max(-0.5, min(0.5, (P(1)*(Target(1) - s(1)) + (kd(1,1)- D(1))*s(7))/nFB));
        phi   = max(-0.5, min(0.5, -(P(2)*(Target(2) - s(2)) + (kd(2,2) - D(2))*s(8))/nFB));
        xi    = 0;
